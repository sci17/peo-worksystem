...
(() => {
    ...
    const upsertMaintenanceTaskFromAdminRecord = (adminRecord, sourceDivisionKey = "") => {
        ...
        const state = readMaintenanceState();
        const dismissedIds = Array.isArray(state?.dismissedAdminTaskIds)
            ? state.dismissedAdminTaskIds.map((id) => String(id || "").trim()).filter(Boolean)
            : [];
        if (dismissedIds.includes(adminRecordId)) {
            return;
        }
        const nextRow = {
            adminRecordId,
...
const portalDomReady = () => {
    ...
    const normalizeBillingHistoryStatus = (value) => {
	    ...
	    const buildBillingHistoryEntry = (record, changedAt) => {
	        const receivedFrom = String(record.received_from || record.received_by || "").trim();
	        const snapshot = {
	            ...
	            // Backward-compat for older stored snapshots/records
    ...
    const buildBillingHistoryCell = (record) => {
        ...
        const historyItems = historyEntries
            ...
            .map((entry, indexFromLatest) => {
                ...
                if (entry.date_received) {
	                ...
	                if (entry.received_from || entry.received_by) {
	                    detailParts.push(entry.received_from || entry.received_by);
	                }
	    ...
	    const resolveAdminDivisionKey = (value) => {
	        const normalized = String(value || "").trim().toLowerCase().replace(/[^a-z0-9]/g, "");
	        if (!normalized) return "";
	        if (normalized.includes("planning")) return "planning";
	        if (normalized.includes("construction")) return "construction";
	    ...
	    const labelForAdminDivisionKey = (key) => ADMIN_DIVISION_LABELS[String(key || "")] || "";
        ...
        normalized.billing_status = normalized.billing_status || normalized.doc_status || "Draft";
	        ...
	        if (!String(normalized.received_from || "").trim()) {
    ...
    const enableAdminDivisionCardFloat = () => {
	    ...
	    const refreshBillingCounters = () => {
	        ...
	        const statuses = rows.map((row) => normalizeStatus(row.children[8]?.textContent));
	        const total = rows.length;
    ...
    const toggleBillingTypeOtherField = (form) => {
	    ...
	    const toggleBillingReceivedFields = (form) => {
	        ...
	        const receivedFromWrap = form?.querySelector('[data-admin-received-from-wrap]');
	        if (!statusSelect || !dateWrap || !receivedFromWrap) return;

	        const normalizedStatus = normalizeStatus(statusSelect.value);
	        const shouldShow = normalizedStatus === "approved" || normalizedStatus === "received";
	        dateWrap.classList.toggle("pa-form-hidden-field", !shouldShow);
	        receivedFromWrap.classList.toggle("pa-form-hidden-field", !shouldShow);
	    };

	    const setAdminModalSectionDisabled = (form, sectionKey, disabled) => {
	        if (!(form instanceof HTMLFormElement)) return;
	        const section = form.querySelector(`[data-admin-form-section="${sectionKey}"]`);
	        if (!section) return;
	    ...
	    const applyAdminNewDocumentModalInteractivity = (form, mode) => {
	        if (!(form instanceof HTMLFormElement)) return;

	        // Reset: everything editable.
	        ["details", "routing", "billing"].forEach((key) => setAdminModalSectionDisabled(form, key, false));
	        ...
	        if (mode === "edit_billing") {
	            ...
	            setAdminModalSectionDisabled(form, "details", true);
	            setAdminModalSectionDisabled(form, "routing", true);
	            ...
	            if (scannedFileInput instanceof HTMLInputElement) {
	                scannedFileInput.disabled = false;
	            }
	        }
        ...
        enhanceAdminFormSelects(form);
	        const recordForForm = {
	            ...record,
	            received_from: record.received_from || record.received_by || "",
    ...
    const applyDocumentFilters = () => {
        ...
        let visibleCount = 0;
	        ...
	        rows.forEach((row) => {
	            const statusValue = normalizeStatus(row.children[8]?.textContent || "");
	            const rowText = Array.from(row.querySelectorAll("td"))
	                .map((cell) => cell.textContent || "")
	                .join(" ")
	                .toLowerCase();
    ...
    const addDocumentRow = (values, options = {}) => {
        ...
        if (emptyRow) emptyRow.remove();
	        ...
	        const row = document.createElement("tr");
	        row.dataset.recordId = recordId;
	        const contractAmount = values.contract_amount || "-";
	        const revisedContractAmount = values.revised_contract_amount || values.contract_amount || "-";
	        const receivedFrom = values.received_from
	            || (labelForAdminDivisionKey(resolveAdminDivisionKey(values.__submitted_from_division)) || "")
	            || values.received_by
	            || "-";
    ...
    const restoreAdminDivisionRecords = () => {
        ...
        if (scannedFile instanceof File && scannedFile.size > 0) {
	        ...
	        const isEditing = Boolean(options.recordId);
	        if (!isEditing) {
	            values.status = values.status || "Draft";
	            values.division = values.division || "Admin";
	            values.doc_type = values.doc_type || "Other";
	        }
	        if (Object.prototype.hasOwnProperty.call(values, "received_from") || Object.prototype.hasOwnProperty.call(values, "received_by")) {
	            const normalizedReceivedFrom = String(values.received_from || values.received_by || "").trim();
	            if (normalizedReceivedFrom) {
	                values.received_from = normalizedReceivedFrom;
	            } else {
	                delete values.received_from;
	            }
	        }
	        const shouldAutoSetDateReceived = !isEditing || options.tableType === "billing";
	        if (shouldAutoSetDateReceived && !String(values.date_received || "").trim()) {
	            values.date_received = getLocalIsoDate();
	        }
        ...
        if (options.recordId) {
            ...
            if (scannedFileAttachment) {
	            ...
	            if (tableType === "billing") {
	                ...
	                if (!String(updatedRecord.received_from || "").trim()) {
	                    const submittedKey = resolveAdminDivisionKey(updatedRecord.__submitted_from_division);
	                    updatedRecord.received_from = labelForAdminDivisionKey(submittedKey) || String(updatedRecord.received_by || "").trim();
	                }
	                if (!String(updatedRecord.date_received || "").trim()) {
	                    updatedRecord.date_received = toInputDate(currentRecord.__submitted_at) || getLocalIsoDate();
	                }
    ...
    const buildNewDocumentModal = () => {
            ...
            <div class="pa-doc-modal">
                <div class="pa-doc-modal-head">
	                ...
	                <form id="admin-new-document-form" class="pa-doc-form">
	                    <div class="pa-doc-form-section" data-admin-form-section="details">
	                    ...
	                    <div class="pa-doc-form-section" data-admin-form-section="routing">
                        <div class="pa-doc-form-section-head">
                            <div class="pa-doc-form-section-icon" aria-hidden="true">
	                    ...
	                    <div class="pa-doc-form-section" data-admin-form-section="billing">
                        <div class="pa-doc-form-section-head">
                            <div class="pa-doc-form-section-icon" aria-hidden="true">
                        ...
                        <div class="pa-doc-form-grid">
                            ...
                            <label class="pa-doc-field pa-form-hidden-field" data-admin-received-date-wrap>
	                            ...
	                            <label class="pa-doc-field pa-form-hidden-field" data-admin-received-from-wrap>
	                                <span>Received From</span>
	                                <select name="received_from">
	                                    <option value="">Select division</option>
	                                    <option>Planning Division</option>
	                                    <option>Construction Division</option>
	                                    <option>Quality Division</option>
	                                    <option>Maintenance Division</option>
        ...
        setFormStatusOptionsByMode(form, "documents");
	        ...
	        toggleBillingReceivedFields(form);
	        setExistingScannedFileName(form, "");
	        applyAdminNewDocumentModalInteractivity(form, "create");
	        ...
	        form.addEventListener("submit", async (event) => {
	            ...
	            try {
	                ...
	                const message = (error && typeof error === "object" && "message" in error)
	                    ...
	                    : "";
	                showAdminStatusToast(
	                    message ? `Unable to save the record: ${message}` : "Unable to save the record. Please try again.",
	                    "info"
	                );
	            }
	        });
...
document.addEventListener("DOMContentLoaded", () => {
    ...
    const ensureTaskPersonnelDivisionDropdown = () => {
        if (!taskPersonnelModal) {
            return;
        }

        const divisionDropdown = taskPersonnelModal.querySelector('.js-road-dropdown[data-input-target="division"]');
        if (!(divisionDropdown instanceof HTMLElement)) {
            return;
        }
        if (divisionDropdown.dataset.dropdownBound === "1") {
            return;
        }
        divisionDropdown.dataset.dropdownBound = "1";

        const trigger = divisionDropdown.querySelector(".dropdown-trigger");
        const label = divisionDropdown.querySelector(".dropdown-label");
        const dropdownMenu = divisionDropdown.querySelector(".dropdown-menu");

        const closeWithinModal = (current = null) => {
            taskPersonnelModal.querySelectorAll(".js-road-dropdown.is-open").forEach((node) => {
                ...
                const nodeTrigger = node.querySelector(".dropdown-trigger");
                if (nodeTrigger) {
        ...
        if (trigger) {
            trigger.addEventListener("click", (event) => {
                ...
                const opening = !divisionDropdown.classList.contains("is-open");
                closeWithinModal(divisionDropdown);
                divisionDropdown.classList.toggle("is-open", opening);
        ...
        if (dropdownMenu) {
            dropdownMenu.addEventListener("click", (event) => {
                ...
                divisionDropdown.querySelectorAll(".dropdown-option").forEach((item) => item.classList.remove("is-selected"));
                option.classList.add("is-selected");

                const selectedValue = String(option.dataset.value || option.textContent || "").trim();
                if (label) {
                    label.textContent = selectedValue || "Select division";
                }
                const hiddenInput = taskPersonnelForm?.elements?.namedItem("division");
                if (hiddenInput instanceof HTMLInputElement) {
                ...
                divisionDropdown.classList.remove("is-open");
                if (trigger) {
                    trigger.setAttribute("aria-expanded", "false");
                }
            });
        ...
        // Close dropdown if user clicks elsewhere inside the modal.
        taskPersonnelModal.addEventListener("click", (event) => {
            if (!(event.target instanceof Node)) {
                return;
            }
            if (divisionDropdown.contains(event.target)) {
                return;
            }
            divisionDropdown.classList.remove("is-open");
            if (trigger) {
    ...
    const openTaskPersonnelModal = () => {
        ...
        ensureTaskPersonnelDivisionDropdown();
        if (taskPersonnelForm) {
            taskPersonnelForm.reset();
        }
        resetTaskPersonnelDivision();
    ...
    const openTaskModalForEdit = (row) => {
        ...
        if (assignedInput instanceof HTMLInputElement) {
            assignedInput.value = row.dataset.assignedPersonnel || "";
        }
    ...
    if (taskPersonnelForm) {
        taskPersonnelForm.addEventListener("submit", (event) => {
            ...
            let division = (formData.get("division") || "").toString().trim();

            // Some pages render the dropdown before the generic dropdown handler binds,
            // so fall back to the selected option / label if the hidden input is empty.
            if (!division && taskPersonnelModal) {
                const divisionDropdown = taskPersonnelModal.querySelector('.task-personnel-dropdown[data-input-target="division"]');
                const selected = divisionDropdown?.querySelector(".dropdown-option.is-selected");
                division = String(selected?.dataset?.value || selected?.textContent || "").trim();
                if (!division) {
                    const label = divisionDropdown?.querySelector(".dropdown-label");
                    const labelText = String(label?.textContent || "").trim();
                    if (labelText && labelText.toLowerCase() !== "select division") {
                        division = labelText;
                    }
                }
                if (division) {
                    const hiddenDivision = taskPersonnelForm.elements.namedItem("division");
                    if (hiddenDivision instanceof HTMLInputElement) {
                        hiddenDivision.value = division;
                    }
                }
            }
    ...
    const updateAdminRecordStatus = (recordId, statusLabel) => {
	    ...
	    if (taskForm && taskTableBody) {
            ...
            const assignedPersonnel = (formData.get("assigned_to") || "").toString().trim();
            const notes = (formData.get("notes") || "").toString().trim();
                ...
                const contractorValue = String(editingTaskRow?.dataset.assignedTo || "").trim();
    ...
    const buildMaintenanceIncomingTaskFromAdminRecord = (adminRecord) => {
        ...
        const contractor = String(record.contractor || "").trim();
        const allocation = String(record.division || "-").trim() || "-";
        const billingType = String(record.billing_type || record.doc_type || record.type || "").trim();
        const amount = String(record.revised_contract_amount || record.contract_amount || "").trim() || "-";
        const dueDateIso = String(
            record.maintenance_sent_date
            || record.date_released_admin
        ...
        const receiveFrom = String(record.received_from || record.received_by || "Admin Division").trim() || "Admin Division";
        const createdAt = String(record.__submitted_at || record.__updated_at || "").trim();
        const updatedAt = String(record.__updated_at || record.__submitted_at || "").trim() || createdAt;
        ...
        return {
            ...
            receiveFrom,
            allocation,
            notes: String(record.description || record.remarks || "").trim(),
            createdAt,
            updatedAt,
            route: "Incoming",
        };
    ...
    const serializeSingleTaskRow = (row) => {
        ...
        return {
            ...
            assignedPersonnel: String(row.dataset.assignedPersonnel || "").trim(),
    ...
    function createTaskRowElement(record) {
        ...
        row.dataset.assignedPersonnel = assignedPersonnelLabel;
        ...
        row.innerHTML = `
            ...
            <td>${buildTaskAssignedMarkup(assigneeLabel)}</td>
            <td>
            ...
            <td class="task-remarks-cell">
                <div class="task-remarks-content">
                    ...
                    <button type="button" class="task-remarks-assign-btn js-task-assign-personnel" aria-label="Assign personnel" title="Assign personnel">
    ...
    function persistMaintenanceState() {
        ...
        const payload = {
            ...
            // Admin-allocated tasks can be re-seeded from the Admin store; keep a "dismissed" list so deletes stick.
            dismissedAdminTaskIds: Array.isArray(dismissedAdminTaskIds) ? dismissedAdminTaskIds : [],
        };
    ...
    function restoreMaintenanceState() {
        ...
        if (!savedState || typeof savedState !== "object") {
            savedState = { ...baseState };
        } else {
            savedState = { ...baseState, ...savedState };
        }
        ...
        dismissedAdminTaskIds = Array.from(
            new Set(
                savedState.dismissedAdminTaskIds
                    .map((id) => String(id || "").trim())
                    .filter(Boolean)
            )
        );
        savedState.dismissedAdminTaskIds = dismissedAdminTaskIds;

        if (dismissedAdminTaskIds.length) {
            const beforeCount = savedState.taskRows.length;
            savedState.taskRows = savedState.taskRows.filter((row) => {
                const id = String(row?.adminRecordId || "").trim();
                return !id || !dismissedAdminTaskIds.includes(id);
            });
            if (savedState.taskRows.length !== beforeCount) {
                writeMaintenanceState({ ...savedState });
            }
        }
        ...
        if (adminRecordById.size) {
            ...
            adminRecordById.forEach((record, recordId) => {
                const normalizedRecordId = String(recordId || "").trim();
                if (normalizedRecordId && dismissedAdminTaskIds.includes(normalizedRecordId)) {
                    return;
                }
                const divisionKey = normalizeTaskDivisionFilterKey(record?.division);
                if (divisionKey !== "maintenance") {
                    return;
                }
                const taskRow = buildMaintenanceIncomingTaskFromAdminRecord(record);
                if (!taskRow) {
                    return;
                }
        ...
        if (taskTableBody && Array.isArray(savedState.taskRows)) {
            ...
            savedState.taskRows.forEach((record) => {
                ...
                if (!dueDateIso && adminRecordId && adminRecordById.has(adminRecordId)) {
                    ...
                    if (!route) {
                        route = normalizeTaskDivisionFilterKey(adminRecord?.division) === "maintenance" ? "Incoming" : "Outgoing";
                    }
                }
                ...
                taskTableBody.append(createTaskRowElement({
                    ...
                    assignedPersonnel: String(record.assignedPersonnel || "").trim(),
                    dueDateIso,
    ...
    const syncTaskManagementFromAdminDivision = (adminRecords) => {
        ...
        records.forEach((adminRecord) => {
            ...
            if (!adminRecordId) return;
            if (normalizeTaskDivisionFilterKey(adminRecord?.division) !== "maintenance") return;
            ...
            if (currentRow instanceof HTMLTableRowElement) {
                ...
                if (assignedPersonnel) {
                    nextRowData.assignedPersonnel = assignedPersonnel;
                }
                const notes = String(currentRow.dataset.notes || "").trim();
                if (notes) {
                    nextRowData.notes = notes;
                }
            }
    ...
    let taskAssignPopover = null;
    let taskAssignPopoverRow = null;
    let taskAssignPopoverCleanup = null;

    const closeTaskAssignPopover = () => {
        if (typeof taskAssignPopoverCleanup === "function") {
            taskAssignPopoverCleanup();
        }
        taskAssignPopoverCleanup = null;
        taskAssignPopoverRow = null;
        if (taskAssignPopover) {
            taskAssignPopover.remove();
        }
        taskAssignPopover = null;
    };

    const assignPersonnelToTaskRow = (row, fullName) => {
        const normalizedName = String(fullName || "").trim();
        if (!(row instanceof HTMLTableRowElement)) {
            return;
        }
        if (!normalizedName) {
            showRoadUploadStatusToast("Please select a personnel name.", "warning");
            return;
        }

        const record = serializeSingleTaskRow(row);
        if (!record) {
            return;
        }

        const nowIso = new Date().toISOString();
        record.assignedPersonnel = normalizedName;
        record.updatedAt = nowIso;
        if (!record.createdAt) {
            record.createdAt = nowIso;
        }
        if (!record.dueDateDisplay && record.dueDateIso) {
            record.dueDateDisplay = formatTaskDateValue(record.dueDateIso);
        }
        ...
        showRoadUploadStatusToast(`Personnel assigned: ${normalizedName}`, "success");
    };

    const clearPersonnelForTaskRow = (row) => {
        if (!(row instanceof HTMLTableRowElement)) {
            return;
        }
        const record = serializeSingleTaskRow(row);
        ...
        persistMaintenanceState();
        closeTaskAssignPopover();
        showRoadUploadStatusToast("Personnel cleared.", "success");
    };

    const getPersonnelCandidatesForTaskRow = (row) => {
        const records = Array.isArray(personnelRecords) ? personnelRecords : [];
        if (!records.length) {
            return [];
        }
        const allocationLabel = String(row?.dataset?.allocation || row?.dataset?.divisionLabel || "").trim();
        const allocationKey = normalizeTaskDivisionFilterKey(allocationLabel);
        const matches = records.filter((record) => normalizeTaskDivisionFilterKey(record?.division) === allocationKey);
        return matches.length ? matches : records;
    };

    const openTaskAssignPopover = (row, anchor) => {
        if (!(row instanceof HTMLTableRowElement) || !(anchor instanceof HTMLElement)) {
            return;
        }

        const candidates = getPersonnelCandidatesForTaskRow(row);
        if (!candidates.length) {
            openTaskPersonnelModal();
            showRoadUploadStatusToast("Please add personnel first.", "warning");
            return;
        }

        closeTaskAssignPopover();
        taskAssignPopoverRow = row;

        const popover = document.createElement("div");
        popover.className = "task-assign-popover";
        popover.setAttribute("role", "dialog");
        popover.setAttribute("aria-label", "Assign personnel");

        const taskTitle = String(row.dataset.title || row.cells[1]?.textContent || "Task").trim();
        const currentSelection = String(row.dataset.assignedPersonnel || "").trim();

        const buildListMarkup = (items) => {
            if (!items.length) {
                return '<p class="task-assign-empty">No personnel found.</p>';
            }
            return `
                <div class="task-assign-list">
                    ${items.map((record) => {
                        ...
                        const isSelected = currentSelection && name === currentSelection;
                        return `
                            <button type="button" class="task-assign-option${isSelected ? " is-selected" : ""}" data-name="${escapeHtml(name)}">
                                <span class="task-assign-name">${escapeHtml(name)}</span>
                                <span class="task-assign-meta">${escapeHtml(division || "-")}</span>
                            </button>
                        `;
                    }).join("")}
                </div>
        ...
        popover.innerHTML = `
            <div class="task-assign-head">
                ...
                <button type="button" class="task-assign-close" aria-label="Close">&times;</button>
            </div>
            <div class="task-assign-body">
                <input type="text" class="task-assign-search" placeholder="Search personnel..." aria-label="Search personnel">
                <div class="task-assign-results">
                    ${buildListMarkup(candidates)}
                </div>
            </div>
            <div class="task-assign-actions">
                <button type="button" class="task-assign-action task-assign-action--secondary js-task-assign-add">Add personnel</button>
                <button type="button" class="task-assign-action task-assign-action--danger js-task-assign-clear">Clear</button>
            </div>
        `;

        document.body.appendChild(popover);
        taskAssignPopover = popover;

        // Position near the clicked icon, but keep within viewport.
        const rect = anchor.getBoundingClientRect();
        const margin = 10;
        popover.style.left = `${Math.min(window.innerWidth - margin, Math.max(margin, rect.left))}px`;
        popover.style.top = `${Math.min(window.innerHeight - margin, Math.max(margin, rect.bottom + 8))}px`;

        // If it overflows bottom, flip above.
        const afterRect = popover.getBoundingClientRect();
        if (afterRect.bottom > window.innerHeight - margin) {
            const top = Math.max(margin, rect.top - afterRect.height - 8);
            popover.style.top = `${top}px`;
        }
        if (afterRect.right > window.innerWidth - margin) {
            const left = Math.max(margin, window.innerWidth - margin - afterRect.width);
            popover.style.left = `${left}px`;
        }

        const closeBtn = popover.querySelector(".task-assign-close");
        const searchInput = popover.querySelector(".task-assign-search");
        const resultsEl = popover.querySelector(".task-assign-results");
        const addBtn = popover.querySelector(".js-task-assign-add");
        const clearBtn = popover.querySelector(".js-task-assign-clear");

        const renderFiltered = (query) => {
            const q = String(query || "").trim().toLowerCase();
            const filtered = candidates.filter((record) => {
                const name = String(record?.fullName || "").toLowerCase();
                const division = String(record?.division || "").toLowerCase();
                return !q || name.includes(q) || division.includes(q);
            });
            if (resultsEl) {
                resultsEl.innerHTML = buildListMarkup(filtered);
            }
        };
        ...
        popover.addEventListener("click", (event) => {
            ...
            const option = target.closest(".task-assign-option");
            if (option) {
                const name = String(option.getAttribute("data-name") || "").trim();
                assignPersonnelToTaskRow(row, name);
                return;
            }
        });

        if (closeBtn instanceof HTMLButtonElement) {
            closeBtn.addEventListener("click", closeTaskAssignPopover);
        }
        ...
        if (clearBtn instanceof HTMLButtonElement) {
            clearBtn.addEventListener("click", () => clearPersonnelForTaskRow(row));
        }
        ...
        const onScroll = () => closeTaskAssignPopover();
    ...
    if (taskTableBody) {
        taskTableBody.addEventListener("click", async (event) => {
            ...
            const assignButton = target.closest(".js-task-assign-personnel");
            if (assignButton) {
                openTaskAssignPopover(row, assignButton);
                return;
            }
    ...
    if (taskTableBody instanceof HTMLElement) {
        ...
        // Also re-hydrate from the server on focus so other users/devices see newly routed Admin records.
        window.addEventListener("focus", requestMaintenanceHydrate);
...
/* CONSTRUCTION_DIVISION_SCRIPT_START */
    ...
    const formatPercent = (value, options = {}) => {
        ...
        date_completed: "Date Completed",
        revised_contract_cost: "Revised Contract Cost",
        status_previous: "Status Previous (%)",
    ...
    const computeConstructionDerivedFields = (record, reportDate) => {
        const next = { ...(record || {}) };
        const isAdminRouted = String(next.__admin_source || "") === "admin_document";

        const parsePercentValue = (value) => {
            const text = normalizeStringValue(value);
            if (!text) return null;
            const numeric = parseConstructionNumber(text);
        ...
        // Original Expiry Date = NTP Date + C.D (days).
        ...
        const revisedExpiryDate = isAdminRouted ? null : parseConstructionDateValue(next.revised_expiry_date);
        ...
        const existingTimeElapsed = normalizeStringValue(next.time_elapsed);
        const shouldComputeTimeElapsed = isAdminRouted || !existingTimeElapsed;
        if (shouldComputeTimeElapsed && ntpDate && cdDays !== null && cdDays > 0) {
            const plannedDays = cdDays + ((addlCdDays !== null && addlCdDays > 0) ? addlCdDays : 0);
            const completedDate = parseConstructionDateValue(next.date_completed);
            const endDate = completedDate || (reportDate instanceof Date ? reportDate : null);
            const ntpMs = getUtcDateOnlyMs(ntpDate);
            const endMs = endDate ? getUtcDateOnlyMs(endDate) : null;
            const elapsedDays = (ntpMs !== null && endMs !== null) ? Math.abs((endMs - ntpMs) / 86400000) : null;

            if (elapsedDays !== null && elapsedDays > 0 && plannedDays > 0) {
                // Per requirement:
                // - No add'l C.D: (NTP date - Completed date) / C.D
                // - With add'l C.D: (NTP date - Completed date) / (C.D + Add'l C.D)
                // Store as a percent value (0-100+) so completed projects land on ~100%.
                next.time_elapsed = String((elapsedDays / plannedDays) * 100);
            }
        }

        const existingSlippage = normalizeStringValue(next.slippage);
        const shouldComputeSlippage = isAdminRouted || !existingSlippage;
        if (shouldComputeSlippage) {
            const statusCurrentPercent = parsePercentValue(next.status_current);
            const timeElapsedPercent = parsePercentValue(next.time_elapsed);
            if (statusCurrentPercent !== null && timeElapsedPercent !== null && timeElapsedPercent !== 0) {
                // Derived from the requested ratio (current status % / time elapsed %) and converted to a signed variance.
                // 0% means on-schedule, positive is ahead, negative is behind.
                next.slippage = String(((statusCurrentPercent / timeElapsedPercent) * 100) - 100);
            }
        }
    ...
    const getConstructionCurrentMonthKey = () => {
        const now = new Date();
        return `${now.getFullYear()}-${String(now.getMonth() + 1).padStart(2, "0")}`;
    };

    const formatConstructionMonth = (value) => {
        const text = normalizeStringValue(value);
        const monthMatch = text.match(/^(\d{4})-(\d{2})$/);
        if (!monthMatch) return text || "-";
        const parsed = new Date(Number(monthMatch[1]), Number(monthMatch[2]) - 1, 1);
        if (Number.isNaN(parsed.getTime())) return text;
    ...
    const getConstructionCurrentUser = () => {
        const dashboardName = normalizeStringValue(constructionDashboard.dataset.currentUser);
        if (dashboardName) return dashboardName;
        const bodyName = normalizeStringValue(document.body?.dataset?.userName);
        return bodyName || "Local User";
    };

    const normalizeConstructionHistoryEntry = (entry, record) => {
        const normalized = { ...(entry || {}) };
        normalized.changed_at = normalizeStringValue(normalized.changed_at) || new Date().toISOString();
        normalized.changed_by = normalizeStringValue(normalized.changed_by) || getConstructionCurrentUser();
        normalized.update_month = normalizeStringValue(normalized.update_month) || getConstructionCurrentMonthKey();
        normalized.update_type = normalizeStringValue(normalized.update_type) || "edited";
        normalized.changed_fields = Array.isArray(normalized.changed_fields)
            ? normalized.changed_fields.map((field) => normalizeStringValue(field)).filter(Boolean)
            : [];

        CONSTRUCTION_FIELDS.forEach((field) => {
            normalized[field] = normalizeStringValue(
                normalized[field] !== undefined ? normalized[field] : record?.[field]
            );
        });
    ...
    const buildConstructionHistoryEntry = (record, options = {}) => {
        const snapshot = {
            changed_at: options.changedAt || new Date().toISOString(),
            changed_by: normalizeStringValue(options.changedBy) || getConstructionCurrentUser(),
            update_month: normalizeStringValue(options.updateMonth) || getConstructionCurrentMonthKey(),
            update_type: normalizeStringValue(options.updateType) || "edited",
            changed_fields: Array.isArray(options.changedFields)
                ? options.changedFields.map((field) => normalizeStringValue(field)).filter(Boolean)
                : [],
        };

        CONSTRUCTION_FIELDS.forEach((field) => {
            snapshot[field] = normalizeStringValue(record?.[field]);
        });

        return normalizeConstructionHistoryEntry(snapshot, record);
    };

    const ensureConstructionHistory = (record) => {
        const existingHistory = Array.isArray(record?.update_history) ? [...record.update_history] : [];
        if (existingHistory.length) {
            return existingHistory.map((entry) => normalizeConstructionHistoryEntry(entry, record));
        }

        return [
            buildConstructionHistoryEntry(record, {
                updateType: String(record?.__admin_source || "").trim() === "admin_document" ? "routed" : "created",
                changedFields: [],
                changedBy: normalizeStringValue(record?.__created_by) || getConstructionCurrentUser(),
                changedAt: normalizeStringValue(record?.__created_at) || new Date().toISOString(),
            }),
        ];
    };
    ...
    const getConstructionFieldLabel = (field) => {
        return CONSTRUCTION_FIELD_LABELS[field] || field;
    };

    const getConstructionUpdateTypeLabel = (value) => {
        const normalized = normalizeStringValue(value).toLowerCase();
        if (normalized === "monthly_update") return "Monthly Update";
    ...
    const buildConstructionHistoryBadges = (fields) => {
        const items = Array.isArray(fields) ? fields : [];
        if (!items.length) return '<span class="construction-history-badge">Initial record</span>';
        return items
            .map((field) => `<span class="construction-history-badge">${escapeHtml(getConstructionFieldLabel(field))}</span>`)
            .join("");
    };
    ...
    const mapAdminRecordToConstruction = (adminRecord) => {
        ...
        const ntpDateValue = String(
            adminRecord?.date_started
            || adminRecord?.date_started_peo
            || adminRecord?.ntp_date
            || ""
        ).trim();
        const completionDateValue = String(
            adminRecord?.completion_date
            || adminRecord?.date_completed
            || adminRecord?.completed_date
            || ""
        ).trim();
        ...
        const getPeriodCoveredDays = (value, fallbackYear) => {
            const text = normalizeStringValue(value).replace(/[–—]/g, "-").trim();
            if (!text) return "";
            ...
            const parseMonthToken = (token) => {
                ...
                return Object.prototype.hasOwnProperty.call(monthByName, key) ? monthByName[key] : null;
            };

            const rangeYearPattern = /^([a-z]{3,9})\s*-\s*([a-z]{3,9})(?:\s+(\d{4}))?$/i;
            const dualYearPattern = /^([a-z]{3,9})\s*(\d{4})\s*-\s*([a-z]{3,9})\s*(\d{4})$/i;
            const singleMonthPattern = /^([a-z]{3,9})(?:\s+(\d{4}))?$/i;

            const resolveYear = (yearToken) => {
                ...
                if (Number.isFinite(fallback) && fallback >= 1900 && fallback <= 3000) return fallback;
                return new Date().getFullYear();
            };

            const diffDaysInclusive = (startUtcMs, endUtcMs) => {
                if (!Number.isFinite(startUtcMs) || !Number.isFinite(endUtcMs)) return "";
                const diff = Math.floor((endUtcMs - startUtcMs) / 86400000) + 1;
                return Number.isFinite(diff) && diff > 0 ? String(diff) : "";
            };

            const normalized = text.replace(/\s+/g, " ");
            let match = normalized.match(dualYearPattern);
            if (match) {
                const startMonth = parseMonthToken(match[1]);
                const startYear = Number(match[2]);
                const endMonth = parseMonthToken(match[3]);
            ...
            match = normalized.match(rangeYearPattern);
            if (match) {
                const startMonth = parseMonthToken(match[1]);
            ...
            match = normalized.match(singleMonthPattern);
            if (match) {
                const monthIndex = parseMonthToken(match[1]);
                if (monthIndex === null) return "";
                const year = resolveYear(match[2]);
                const dayCount = new Date(Date.UTC(year, monthIndex + 1, 0)).getUTCDate();
                return Number.isFinite(dayCount) && dayCount > 0 ? String(dayCount) : "";
            }

            return "";
        };

        const inferredYear = parseConstructionDateValue(ntpDateValue)?.getFullYear()
            || parseConstructionDateValue(completionDateValue)?.getFullYear()
            || new Date().getFullYear();
        const periodCoveredDays = getPeriodCoveredDays(adminRecord?.period_covered, inferredYear);
        let cdValue = String(periodCoveredDays || "").trim();
        if (!cdValue) {
            const ntpDate = parseConstructionDateValue(ntpDateValue);
            const completionDate = parseConstructionDateValue(completionDateValue);
            const startUtc = getUtcDateOnlyMs(ntpDate);
            const endUtc = getUtcDateOnlyMs(completionDate);
    ...
    const syncAdminRoutedConstructionRecords = (existingRecords) => {
        ...
        const mergedRouted = Array.from(routedBySourceId.values()).map((incoming) => {
            ...
            return {
                ...
                ntp_date: String(incoming.ntp_date || "").trim(),
                cd: String(incoming.cd || "").trim(),
                original_expiry_date: "",
                revised_expiry_date: "",
    ...
    const clampCurrentPage = () => {
	    ...
	    const getCurrentPageRecords = () => {
	        ...
	        return getVisibleRecords().slice(start, start + CONSTRUCTION_PAGE_SIZE);
	    };
    ...
    const updateSelectionControls = () => {
	    ...
	    const renderTable = () => {
	        ...
	        const totalCount = getVisibleRecords().length;
	            ...
	            pagedRecords.forEach((record, index) => {
	                ...
	                const isAdminAssigned = isAdminRoutedRecord(record);
	                const projectName = escapeHtml(toDisplay(record.project_name));
	        ...
	        if (constructionRecordMeta) {
	            constructionRecordMeta.textContent = `${totalCount} record${totalCount === 1 ? "" : "s"}`;
	        }
	        renderPagination();
    ...
    const initializeConstructionRecord = (record, options = {}) => {
        ...
        CONSTRUCTION_FIELDS.forEach((field) => {
            baseRecord[field] = normalizeStringValue(baseRecord[field]);
        });
    ...
    const applyConstructionRecordUpdate = (previousRecord, nextValues, options = {}) => {
        const merged = normalizeConstructionRecord({
            ...previousRecord,
            ...nextValues,
            __id: previousRecord?.__id || nextValues?.__id || createRecordId(),
            __created_at: previousRecord?.__created_at,
            __created_by: previousRecord?.__created_by,
        });
        const changedFields = Array.isArray(options.changedFields)
            ? options.changedFields
            : getConstructionChangedFields(previousRecord, merged);

        if (!changedFields.length) return merged;
    ...
    const setConstructionProjectOverviewReadonly = (isReadonly) => {
        if (!constructionForm) return;
    ...
    const openConstructionModal = (mode = "create", record = null) => {
        ...
        requestAnimationFrame(() => {
            const firstField = mode === "edit"
                ? constructionForm?.querySelector('input:not([type="file"]):not([readonly]):not([disabled]), select:not([disabled]), textarea:not([readonly]):not([disabled])')
                : constructionForm?.querySelector('[name="project_name"]');
        ...
        constructionForm.addEventListener("submit", async (event) => {
            ...
            if (editingRecordId) {
                ...
                if (index >= 0) {
                    ...
                    const changedFields = getConstructionChangedFields(previousRecord, nextRecord);
                    if (!changedFields.length) {
                        showPeoGeneralToast("No changes were detected for this construction record.", {
                            title: "Construction Division",
                            variant: "warning",
                        });
                        return;
                    }
                    const shouldSnapshot = ["project_name", "location", "contractor", "status_current", "accomplishment_images"].some((field) =>
...
document.addEventListener("DOMContentLoaded", () => {
    ...
    const titleEl = document.querySelector(".js-construction-project-title");
    const subtitleEl = document.querySelector(".js-construction-project-subtitle");
    const metaEl = projectDashboard.querySelector(".js-construction-project-meta");
    const nameInput = projectDashboard.querySelector(".js-construction-project-name");
    const accomplishmentInput = projectDashboard.querySelector(".js-construction-project-accomplishment");
    const imageInput = projectDashboard.querySelector(".js-construction-project-image-input");
    const imageGallery = projectDashboard.querySelector(".js-construction-project-image-gallery");
    const imageMeta = projectDashboard.querySelector(".js-construction-project-image-meta");
    const removeImageButton = projectDashboard.querySelector(".js-construction-project-remove-image");
    const saveButton = projectDashboard.querySelector(".js-construction-project-save");
    ...
    const disableEditor = () => {
        if (accomplishmentInput) accomplishmentInput.disabled = true;
        if (imageInput) imageInput.disabled = true;
        if (removeImageButton) removeImageButton.disabled = true;
    ...
    const toast = (message, options = {}) => {
        ...
        // Fallback for pages that do not load toast helpers.
    ...
    const renderGallery = (items) => {
        ...
        if (imageMeta) {
            imageMeta.textContent = images.length
                ? `${images.length} image${images.length === 1 ? "" : "s"} selected.`
                : "No images uploaded.";
        }
        if (removeImageButton) {
            removeImageButton.disabled = images.length === 0;
        }
        ...
        images.forEach((img, index) => {
            ...
            thumb.src = url;
            thumb.alt = String(img?.name || "Uploaded project image");
    ...
    const normalizeHistoryItems = (items, fallbackRecord) => {
        const list = Array.isArray(items) ? items : [];
        const recordFallback = fallbackRecord || {};

        const normalized = list
            .map((entry) => {
                const snapshot = entry && typeof entry === "object" ? entry : {};
                const monthKey = String(snapshot.month || snapshot.update_month || "").trim();
                const savedAt = String(snapshot.saved_at || snapshot.changed_at || "").trim();
                ...
                return {
                    ...
                    month: monthKey || getMonthKey(savedAt ? new Date(savedAt) : new Date()),
                    saved_at: savedAt || new Date().toISOString(),
                    project_name: String(snapshot.project_name || recordFallback.project_name || "").trim(),
                    location: String(snapshot.location || recordFallback.location || "").trim(),
                    contractor: String(snapshot.contractor || recordFallback.contractor || "").trim(),
    ...
    if (!recordId) {
        if (metaEl) metaEl.textContent = "Missing project id.";
        showWarning("No project id was provided. Please go back and click a project name from the register.");
        disableEditor();
        return;
    }

    const records = readStoredRecords();
    const recordIndex = records.findIndex((item) => String(item?.__id || "") === recordId);
    if (recordIndex < 0) {
        if (metaEl) metaEl.textContent = "Project not found.";
        showWarning("This project could not be found in local storage. It may have been deleted or not yet created on this device.");
        disableEditor();
        return;
    }
    ...
    if (subtitleEl) subtitleEl.textContent = "Update accomplishment progress and attach an image.";
    if (nameInput instanceof HTMLInputElement) nameInput.value = projectName;

    if (metaEl) {
        const location = String(record.location || "").trim();
        const contractor = String(record.contractor || "").trim();
        const parts = [location, contractor].filter(Boolean);
        metaEl.textContent = parts.length ? parts.join(" · ") : `Record ID: ${recordId}`;
    }

    const percentRaw = String(record.status_current ?? "").trim();
    const percentValue = Number.parseFloat(percentRaw.replace("%", ""));
    if (accomplishmentInput instanceof HTMLInputElement && Number.isFinite(percentValue)) {
        accomplishmentInput.value = String(percentValue);
    }
    ...
    if (imageGallery) {
        imageGallery.addEventListener("click", (event) => {
            ...
            const idxRaw = String(button.dataset.imageIndex || "").trim();
            if (!idxRaw) return;
    ...
    if (saveButton) {
        saveButton.addEventListener("click", () => {
            const nextPercentRaw = accomplishmentInput instanceof HTMLInputElement
                ...
                : "";
            const nextPercent = nextPercentRaw === "" ? null : Number.parseFloat(nextPercentRaw);

            if (nextPercent !== null && (!Number.isFinite(nextPercent) || nextPercent < 0 || nextPercent > 100)) {
                toast("Accomplishment must be a number between 0 and 100.", {
                    title: "Construction Project",
                    variant: "warning",
                });
                return;
            ...
            const nextRecord = { ...(records[recordIndex] || {}) };
            ...
            const historyEntry = {
                ...
                month: getMonthKey(new Date(nextRecord.__updated_at)),
                project_name: String(nextRecord.project_name || "").trim(),
            ...
            accomplishmentHistory = normalizeHistoryItems(
                [historyEntry, ...(Array.isArray(nextRecord.accomplishment_history) ? nextRecord.accomplishment_history : [])],
                nextRecord
            );
            nextRecord.accomplishment_history = accomplishmentHistory;

            records[recordIndex] = nextRecord;
            const wrote = writeStoredRecords(records);

            if (!wrote) {
                toast("Could not save to local storage (storage may be full). Try removing some images and save again.", {
                    title: "Construction Project",
                    variant: "warning",
                });
                return;
...
/* CONSTRUCTION_PROJECT_DASHBOARD_SCRIPT_END */
...
document.addEventListener("DOMContentLoaded", () => {
        ...
        const mergedExisting = records.reduce((acc, record) => {
            ...
            // Preserve Planning-side edits for fields managed in the Planning document register.
	            acc.push({
	                ...
	                contract_amount: String(pickPlanningEditedValue(record?.contract_amount, mapped.contract_amount || "")).trim(),
	                amount: String(pickPlanningEditedValue(record?.amount, mapped.amount || "")).trim(),
	                revised_contract_amount: String(pickPlanningEditedValue(record?.amount, mapped.amount || "")).trim(),
    ...
    const mapPlanningDocStatusToPpaStatus = (value) => {
        const normalized = normalizePlanningDocStatus(value);
        if (normalized === "Approved") return "Approved";
    ...
    const ensurePlanningPpaLocationColumn = () => {
        if (!(planningPpaTableBody instanceof HTMLTableSectionElement)) return;
        const table = planningPpaTableBody.closest("table");
        const headRow = table?.querySelector("thead tr");
        if (headRow instanceof HTMLTableRowElement) {
            const headers = Array.from(headRow.querySelectorAll("th"));
            const hasLocation = headers.some((th) => String(th.textContent || "").trim().toLowerCase() === "location");
            if (!hasLocation) {
                const locationTh = document.createElement("th");
                locationTh.textContent = "Location";
                headRow.insertBefore(locationTh, headers[1] || null);
            }
        }

        planningPpaTableBody.querySelectorAll("tr").forEach((row) => {
            if (!(row instanceof HTMLTableRowElement)) return;
            if (row.dataset.ppaEmptyRow === "true") return;
            if (row.cells.length === 5) {
                const locationTd = document.createElement("td");
                locationTd.textContent = "-";
                row.insertBefore(locationTd, row.cells[1] || null);
            }
        });
    };

    const syncPpaFromPlanningDocuments = (records) => {
        if (!(planningPpaTableBody instanceof HTMLTableSectionElement)) return;
        ensurePlanningPpaLocationColumn();
        const docs = Array.isArray(records) ? records : [];

        const existingRows = Array.from(
            planningPpaTableBody.querySelectorAll('tr[data-ppa-source="planning_doc"]')
        ).filter((row) => row instanceof HTMLTableRowElement);

        const rowByDocId = new Map(
            existingRows.map((row) => [String(row.dataset.planningDocId || "").trim(), row])
        );
        ...
        const incoming = docs
            .map((doc) => {
                ...
                const name = String(doc?.document_name || "").trim() || "(Untitled Project)";
                const displayId = String(doc?.slip_no || id).trim();
                const location = String(doc?.location || "").trim() || "-";
                const fundSource = String(
                    resolvePlanningBudgetAllocationValue(doc?.budget_allocation)
                    || doc?.budget_allocation
                    || "-"
                ).trim() || "-";
                const status = mapPlanningDocStatusToPpaStatus(doc?.status);
                const amountNumber = parsePlanningDocAmount(doc?.amount);
                const amountText = String(doc?.amount || "").trim();
                const amountDisplay = amountText ? formatPhp(amountNumber) : "-";

                return { id, name, displayId, location, fundSource, status, amountDisplay };
            })
            .filter(Boolean)
            .sort((a, b) => String(a.name).localeCompare(String(b.name)));

        incoming.forEach((doc) => {
            const existingRow = rowByDocId.get(doc.id);
            const row = existingRow || document.createElement("tr");
            row.dataset.ppaSource = "planning_doc";
            row.dataset.planningDocId = doc.id;
            ...
            if (!existingRow) {
                row.innerHTML = `
                    ...
                    <td></td>
                    <td></td>
                    <td class="planning-ppa-amount"></td>
                    <td class="planning-ppa-actions"></td>
                `;
            }

            const firstCell = row.cells[0];
            const locationCell = row.cells[1];
            const fundCell = row.cells[2];
            const statusCell = row.cells[3];
            const amountCell = row.cells[4];
            const actionsCell = row.cells[5];

            const titleEl = firstCell?.querySelector("strong");
            if (titleEl instanceof HTMLElement) {
                titleEl.textContent = doc.name;
            }
            const idEl = firstCell?.querySelector("small");
            if (idEl instanceof HTMLElement) {
                idEl.textContent = `ID: ${doc.displayId}`;
            }
            if (locationCell) {
                locationCell.textContent = doc.location;
            }
            if (fundCell) {
                fundCell.textContent = doc.fundSource;
            }

            if (statusCell) {
                const statusSpan = document.createElement("span");
                statusSpan.className = `planning-ppa-status ${getPpaStatusClassName(doc.status)}`;
            ...
            if (amountCell) {
                amountCell.textContent = doc.amountDisplay;
            }
        ...
        existingRows.forEach((row) => {
            const docId = String(row.dataset.planningDocId || "").trim();
            if (!docId || !keepDocIds.has(docId)) {
                row.remove();
            }
        });
    };
    ...
    const syncPpaTableState = () => {
        ...
        if (dataRows.length) {
            ...
            row.innerHTML = '<td colspan="6">No PPA records available.</td>';
            planningPpaTableBody.appendChild(row);
        }
    ...
    const getPlanningPpaBudgetAllocationOptions = () => {
        ...
        return planningBudgetRecords
            ...
            .map((record) => {
                ...
                const usedAllocation = getUsedAllocationTotalForBudget(budgetName);
                const metrics = getBudgetMetrics(record, usedAllocation);
                return {
                    budgetName,
                    remainingBudget: Number(metrics?.remainingBudget ?? 0),
                    createdAt: Number(record?.createdAt || 0),
                };
            })
            .filter((item) => item && item.budgetName && item.remainingBudget > 0)
            .sort((a, b) => Number(b.createdAt) - Number(a.createdAt))
            .map((item) => item.budgetName);
    };

    const refreshPlanningPpaBudgetAllocationSelect = (preferredValue = "") => {
        if (!(ppaBudgetAllocationSelect instanceof HTMLElement)) return;

        const menu = ppaBudgetAllocationSelect.querySelector(".js-planning-ppa-form-menu");
        const input = ppaBudgetAllocationSelect.querySelector(".js-planning-ppa-form-input");
        if (!(menu instanceof HTMLElement) || !(input instanceof HTMLInputElement)) return;

        const placeholder = "Select approved budget with remaining balance";
        const options = getPlanningPpaBudgetAllocationOptions();
        const current = String(preferredValue || input.value || "").trim();
        const nextValue = options.includes(current) ? current : "";

        const buildOption = ({ value, label, selected, disabled }) => {
            const isSelected = selected ? " is-selected" : "";
            const isDisabled = disabled ? " disabled" : "";
            return `
                <li>
                    <button type="button" class="planning-ppa-form-option${isSelected}" data-value="${escapeHtml(value)}"${isDisabled}>
                        <span>${escapeHtml(label)}</span>
        ...
        const rows = [
            buildOption({ value: "", label: placeholder, selected: nextValue === "", disabled: false }),
            ...(options.length
                ? options.map((value) => buildOption({ value, label: value, selected: value === nextValue, disabled: false }))
                : [buildOption({ value: "", label: "No approved budgets available.", selected: false, disabled: true })]
            ),
        ];

        menu.innerHTML = rows.join("");
        setPpaFormSelectValue(ppaBudgetAllocationSelect, nextValue);
    };
    ...
    const openPpaModal = () => {
        ...
        refreshPlanningPpaBudgetAllocationSelect("");
        window.requestAnimationFrame(() => {
    ...
    const refreshPlanningDocBudgetAllocationSelect = (selectedValue = "") => {
	    ...
	    const syncPlanningDocBudgetOtherField = () => {
	        if (!(planningDocBudgetAllocationSelect instanceof HTMLSelectElement)
	            ...
	            || !(
	                ...
	                || planningDocBudgetOtherInput instanceof HTMLTextAreaElement
	            )) {
	            return;
	        }
	        const isOthers = planningDocBudgetAllocationSelect.value === "Others";
	        planningDocBudgetOtherWrap.hidden = !isOthers;
	        planningDocBudgetOtherWrap.style.display = isOthers ? "" : "none";
	    ...
	    const setPlanningDocReadonlyFields = (form, isReadonly) => {
	        if (!(form instanceof HTMLFormElement)) return;
        ...
        const normalizedBudget = resolvePlanningBudgetAllocationValue(record.budget_allocation)
	        ...
	        setValue("budget_allocation", normalizedBudget);
		        ...
		        setPlanningDocReadonlyFields(planningDocEditForm, true);
	 	        if (planningRouteDivisionSelect instanceof HTMLSelectElement) {
	 	            planningRouteDivisionSelect.value = "";
	 	        }
            if (planningRouteFileInput instanceof HTMLInputElement) {
        ...
        ppaFormSelects.forEach((selectRoot) => {
            ...
            if (menu instanceof HTMLElement) {
                menu.addEventListener("click", (event) => {
                    const target = event.target;
                    if (!(target instanceof HTMLElement)) return;
                    const option = target.closest(".planning-ppa-form-option");
                    if (!(option instanceof HTMLElement)) return;
    ...
    if (planningDocEditForm instanceof HTMLFormElement) {
            ...
            syncPpaTableState();
            syncPlanningDocumentStatusToAdmin(updatedRecord);
            closePlanningDocModal();
    ...
    if (editPpaForm instanceof HTMLFormElement) {
        editPpaForm.addEventListener("submit", (event) => {
            ...
            const fundCell = editingPpaRow.cells[2];
            const statusCell = editingPpaRow.cells[3];
            const amountCell = editingPpaRow.cells[4];
    ...
    if (projectBoard) {
        ...
        const projectForm = projectBoard.querySelector("[data-project-form]");
	        ...
	        const projectPrevButton = projectBoard.querySelector("[data-project-prev]");
	        const projectNextButton = projectBoard.querySelector("[data-project-next]");
        ...
        const closeProjectButtons = projectBoard.querySelectorAll(".js-project-close-modal");
	        ...
	        const projectFilterDropdownSyncers = [];
		        ...
		        const readProjectStoreSeed = () => {
		            if (projectStoreSeedCache !== null) return projectStoreSeedCache;
		            const seedNode = document.getElementById("peo-project-store-seed");
		            if (!seedNode) {
		                projectStoreSeedCache = {};
		                return projectStoreSeedCache;
		            }
		            ...
		            projectStoreSeedCache = seed && typeof seed === "object" ? seed : {};
		            return projectStoreSeedCache;
		        };

		        const readSeedArray = (storeKey) => {
		            const seed = readProjectStoreSeed();
		            const data = seed && typeof seed === "object" ? seed[storeKey] : null;
		            return Array.isArray(data) ? data : [];
		        };

		        const readSeedObject = (storeKey) => {
		            const seed = readProjectStoreSeed();
		            const data = seed && typeof seed === "object" ? seed[storeKey] : null;
            ...
            const safeJsonParse = (value) => {
                try {
		            ...
		            defs.forEach((def) => {
		                const serverData = def.type === "object" ? readSeedObject(def.storeKey) : readSeedArray(def.storeKey);
        ...
        const normalizeConstructionSnapshots = (record) => {
            const base = record && typeof record === "object" ? record : {};
            const raw = Array.isArray(base.accomplishment_history) ? base.accomplishment_history : [];

            const normalized = raw
                .map((entry) => {
                    const snap = entry && typeof entry === "object" ? entry : {};
                    const savedAt = String(snap.saved_at || snap.changed_at || "").trim() || "";
                    const month = String(snap.month || snap.update_month || "").trim() || getMonthKey(savedAt || new Date());
                    const percent = String(snap.status_current ?? "").trim();
                    const imageUrl = String(snap.image_data_url || "").trim();
                    const imageName = String(snap.image_name || "").trim();
                    const images = Array.isArray(snap.images) ? snap.images : (imageUrl ? [{ dataUrl: imageUrl, name: imageName }] : []);

                    return {
                        month,
                        saved_at: savedAt || "",
                        project_name: String(snap.project_name || base.project_name || "").trim(),
                        location: String(snap.location || base.location || "").trim(),
                        contractor: String(snap.contractor || base.contractor || "").trim(),
                        status_current: percent,
                        images: images
                            .map((img) => ({
                ...
                .filter((snap) => snap.month);

            normalized.sort((a, b) => String(b.month).localeCompare(String(a.month)) || String(b.saved_at).localeCompare(String(a.saved_at)));
            return normalized;
        };
        ...
        const getProjectRows = () => {
            ...
            if (projectResultsSummary instanceof HTMLElement) {
	            ...
	            divisionSummaryCards.forEach((card) => {
	                ...
	                card.textContent = `${count} project${count === 1 ? "" : "s"}`;
	            });
	        };
	        ...
	        const renderProjectTable = (records) => {
	            ...
	            projectTableBody.innerHTML = list.map((record) => {
	                const isConstruction = String(record.division || "").trim() === "Construction Division";
	                const sourceId = String(record.sourceRecordId || "").trim();
	                const projectName = escapeProjectHtml(record.projectName || "-");
	                const projectNameCell = isConstruction && sourceId
	                    ? `<button type="button" class="project-name-link js-project-open-construction-history" data-construction-source-id="${escapeProjectHtml(sourceId)}">${projectName}</button>`
	                    : projectName;
	                const statusText = escapeProjectHtml(record.statusDisplay || record.status || "In Planning");

	                return `
	                <tr>
	                    <td>${projectNameCell}</td>
	        ...
	        const renderProjectPagination = () => {
	            ...
	            if (projectPageLabel instanceof HTMLElement) {
	                projectPageLabel.textContent = `Page ${currentProjectPage} of ${totalPages}`;
	            }
	            if (projectPrevButton instanceof HTMLButtonElement) {
	                projectPrevButton.disabled = currentProjectPage <= 1;
	            }
	            if (projectNextButton instanceof HTMLButtonElement) {
	                projectNextButton.disabled = currentProjectPage >= totalPages;
	            }
	        };

	        const renderProjectBoardPage = () => {
	            const list = Array.isArray(visibleProjectRecords) ? visibleProjectRecords : [];
	            ...
	            renderProjectTable(list.slice(start, end));
        ...
        const closeConstructionHistoryModal = (persistState = true) => {
	        ...
	        const buildConstructionMonthlyHistoryCards = (constructionRecord) => {
	            const record = constructionRecord && typeof constructionRecord === "object" ? constructionRecord : {};
	            const snapshots = normalizeConstructionSnapshots(record);
	            ...
	            const formatMoney = (value) => {
	                const text = String(value ?? "").trim();
	                if (!text) return "-";
	                if (/php|â‚±/i.test(text)) return text;
	                const numeric = Number(text.replace(/,/g, ""));
	                if (Number.isNaN(numeric)) return text;
	                return `PHP ${numeric.toLocaleString("en-US", { minimumFractionDigits: 2, maximumFractionDigits: 2 })}`;
	            };
		            ...
		            const formatMonthYear = (value) => {
	                    ...
	                    <div class="project-history-month-head">
	                        <div>
	                            ...
	                            <div class="project-history-month-meta">Read-only from Construction Division register</div>
	                        </div>
	                    </div>
	                        ...
	                        <table class="road-table project-history-details-table">
	                            <thead>
	                                    ...
	                                    <th>Revised Completion Date</th>
		                                        <th>${escapeProjectHtml(asOfHeader)}</th>
	                            ...
	                            <tbody>
	                                    ...
	                                    <td>${escapeProjectHtml(formatDate(record.revised_expiry_date))}</td>
		                                        <td>${escapeProjectHtml(statusPercent)}</td>
		                                        <td>${escapeProjectHtml(toDisplay(record.remarks))}</td>
		                                        <td>${escapeProjectHtml(toDisplay(record.contractor))}</td>
		                                    </tr>
	                            </tbody>
	                        </table>
	                    </div>
                ...
                return [{
                    ...
                    saved_at: String(record.__updated_at || record.__created_at || "").trim(),
                    project_name: String(record.project_name || "").trim(),
                    location: String(record.location || "").trim(),
                    contractor: String(record.contractor || "").trim(),
                    status_current: String(record.status_current ?? "").trim(),
                    images,
                }];
            };
        ...
        const openConstructionHistoryModal = (constructionRecord) => {
            ...
            if (constructionHistoryTitle instanceof HTMLElement) {
                constructionHistoryTitle.textContent = "Construction Monthly History";
            }
            if (constructionHistorySubtitle instanceof HTMLElement) {
                constructionHistorySubtitle.textContent = `Monthly snapshots and uploaded images for: ${projectName}`;
            }
            if (constructionHistoryCards instanceof HTMLElement) {
                constructionHistoryCards.innerHTML = buildConstructionMonthlyHistoryCards(record);
            }
        ...
        [projectModal, deleteModal, constructionHistoryModal].forEach((modal) => {
            ...
            modal.addEventListener("click", (event) => {
                ...
                if (modal === constructionHistoryModal) {
                    closeConstructionHistoryModal();
                    return;
                }
                closeDeleteModal();
        ...
        if (projectTableBody instanceof HTMLElement) {
            projectTableBody.addEventListener("click", (event) => {
                const target = event.target;
                if (!(target instanceof HTMLElement)) return;
                const trigger = target.closest(".js-project-open-construction-history");
...
document.addEventListener("DOMContentLoaded", () => {
    ...
    const qualityUploadParticularsDefault = new Set(
        [
            "test result",
            "quality control program",
            "quality control assurance",
            "certificate of no testing",
            "affidavit (inspectorate)",
    ...
    const qualityUploadOutgoingStatusesDefault = new Set(
        ["for action", "in progress", "for release"].map((value) => normalizeText(value))
    );

    const qualityUploadParticulars = (
        qualityForm instanceof HTMLElement
            ? parseCsvSet(qualityForm.dataset.qualityUploadParticulars)
            : new Set()
    );
    if (!qualityUploadParticulars.size) {
        qualityUploadParticularsDefault.forEach((value) => qualityUploadParticulars.add(value));
    }

    const qualityUploadOutgoingStatuses = (
        qualityForm instanceof HTMLElement
            ? parseCsvSet(qualityForm.dataset.qualityUploadOutgoingStatuses)
            : new Set()
    );
    if (!qualityUploadOutgoingStatuses.size) {
        qualityUploadOutgoingStatusesDefault.forEach((value) => qualityUploadOutgoingStatuses.add(value));
    }

    let qualityScanAttachment = null;
    let qualityBaseScanAttachments = [];
    let qualityScanUploadPrompted = false;
    let qualityScanUploadDismissed = false;

    const QUALITY_LOCKED_FIELDS = [
        "received_from",
        "doc_date",
        "doc_no",
        "project_location",
    ...
    const setQualityEditReadonly = (isReadonly) => {
        if (!(qualityForm instanceof HTMLFormElement)) return;
        QUALITY_LOCKED_FIELDS.forEach((fieldName) => {
    ...
    const getQualityFormRadioValue = (fieldName) => {
        if (!(qualityForm instanceof HTMLFormElement)) return "";
        const field = qualityForm.elements.namedItem(fieldName);
    ...
    const getQualityScanUrlValue = () => {
        if (!(qualityForm instanceof HTMLFormElement)) return "";
        const field = qualityForm.elements.namedItem("scan_url");
        if (field instanceof HTMLInputElement || field instanceof HTMLTextAreaElement) {
            return String(field.value || "").trim();
        }
        return "";
    ...
    const qualityHasAnyScan = () => {
        if (qualityScanAttachment) return true;
        if (qualityBaseScanAttachments.length) return true;
        return Boolean(getQualityScanUrlValue());
    };

    const qualityShouldActivateScanUpload = () => {
        const particulars = normalizeText(getQualityFormRadioValue("particulars"));
        const route = normalizeText(getQualityFormRadioValue("route"));
        const status = normalizeText(getQualityFormRadioValue("status"));
        const matchesParticular = particulars && qualityUploadParticulars.has(particulars);
        const matchesOutgoing = route === "outgoing" && qualityUploadOutgoingStatuses.has(status);
        return Boolean(matchesParticular || matchesOutgoing);
    };

    const applyQualityScanAttachmentToRecord = (sourceRecord) => {
        const record = sourceRecord && typeof sourceRecord === "object" ? { ...sourceRecord } : null;
        if (!record) return sourceRecord;
        if (!qualityScanAttachment) return record;

        const mergedAttachments = peoMergeAttachments(peoExtractAttachments(record), [qualityScanAttachment]);
        record.__attachments = mergedAttachments;

        const latest = mergedAttachments.length ? mergedAttachments[mergedAttachments.length - 1] : null;
        if (latest) {
            record.scanned_file_name = latest.name || record.scanned_file_name || "";
            record.scanned_file_type = latest.type || record.scanned_file_type || "";
            record.scanned_file_data = latest.dataUrl || record.scanned_file_data || "";
        }

        return record;
    };

    const maybePromptQualityRouteFileUpload = ({ autoPrompt = false } = {}) => {
        if (!autoPrompt) return;
        if (!(qualityRouteFileInput instanceof HTMLInputElement)) return;
        if (!qualityShouldActivateScanUpload()) return;
        if (qualityHasAnyScan()) return;
        if (qualityScanUploadPrompted || qualityScanUploadDismissed) return;

        qualityScanUploadPrompted = true;
        window.setTimeout(() => {
            if (qualityRouteFileInput instanceof HTMLInputElement && !qualityHasAnyScan()) {
                qualityRouteFileInput.click();
            }
        }, 0);
    };

    const ensureQualityScanAttachmentFromRouteFile = async () => {
        if (qualityScanAttachment) return { ok: true };
        if (!(qualityRouteFileInput instanceof HTMLInputElement)) return { ok: true };
        const selectedFile = qualityRouteFileInput.files && qualityRouteFileInput.files[0];
        if (!(selectedFile instanceof File) || !selectedFile.size) return { ok: true };
        ...
        try {
            ...
            if (!attachment) {
                return { ok: false, error: "Unable to attach file." };
            }
            qualityScanAttachment = attachment;
            return { ok: true };
        } catch (error) {
            return { ok: false, error: "Unable to read the attached file. Please try again." };
        }
    };
    ...
    const buildRecordFromForm = () => {
        ...
        if (!(record.received_from && record.doc_date && record.doc_no)) return null;
        return applyQualityScanAttachmentToRecord(record);
    };
    ...
    if (qualityParticularsSelect instanceof HTMLSelectElement) {
        ...
        qualityBaseScanAttachments = peoExtractAttachments(mode === "edit" ? record : null);
        qualityScanUploadPrompted = false;
        qualityScanUploadDismissed = false;
        ...
        if (deleteQualityButton instanceof HTMLElement) {
            ...
            maybePromptQualityRouteFileUpload({ autoPrompt: false });
        ...
        if (firstField instanceof HTMLElement) {
 	    ...
 	    const closeQualityModal = () => {
 	        ...
 	        editingRecordId = null;
            setQualityEditReadonly(false);
            qualityScanAttachment = null;
            qualityBaseScanAttachments = [];
            qualityScanUploadPrompted = false;
            qualityScanUploadDismissed = false;
 	        if (qualityRouteDivisionSelect instanceof HTMLSelectElement) {
 	            qualityRouteDivisionSelect.value = "";
        ...
        if (qualityRouteFileInput instanceof HTMLInputElement) {
            qualityRouteFileInput.addEventListener("change", async () => {
                qualityScanAttachment = null;
                const result = await ensureQualityScanAttachmentFromRouteFile();
                if (!result.ok) {
                    qualityRouteFileInput.value = "";
                    showPeoGeneralToast(String(result.error || "Unable to read the attached file. Please try again."), {
                        title: "Quality Division",
                        variant: "danger",
                    });
                }
	    ...
	    if (qualityRouteDivisionSelect instanceof HTMLSelectElement) {
 	    ...
 	    if (qualityRouteSubmitButton instanceof HTMLButtonElement) {
	        qualityRouteSubmitButton.addEventListener("click", async () => {
                ...
                const scanResult = await ensureQualityScanAttachmentFromRouteFile();
                if (!scanResult.ok) {
                    showPeoGeneralToast(String(scanResult.error || "Unable to read the attached file. Please try again."), {
                        title: "Quality Division",
                        variant: "danger",
                    });
                    return;
    ...
    if (qualityForm instanceof HTMLFormElement) {
        qualityForm.addEventListener("change", (event) => {
            ...
            if (!(target instanceof HTMLElement)) return;
            if (
                target.matches("[name='particulars'], [name='route'], [name='status']")
                || target.closest("[name='particulars'], [name='route'], [name='status']")
            ) {
                maybePromptQualityRouteFileUpload({ autoPrompt: true });
            }
        });
...
if (document.readyState === "loading") {
    document.addEventListener("DOMContentLoaded", portalDomReady);
} else {
    portalDomReady();
}
	    ...
	    const formatDate = (value) => {
	        ...
	        if (typeof value === "number" && Number.isFinite(value)) {
	            if (value > 59) {
	                const excelEpoch = new Date(Date.UTC(1899, 11, 30));
	                const parsed = new Date(excelEpoch.getTime() + (value * 86400000));
	                if (!Number.isNaN(parsed.getTime())) return toLabel(parsed);
	            }
	            return String(value);
	        }
		    ...
		    const formatMonthYear = (value) => {
			    ...
			    const buildProjectDetailsCard = (record) => {
	                    ...
	                    <table class="road-table project-history-details-table">
	                        ...
	                        <tbody>
		                            <tr>
		                                ...
		                                <td>${escapeHtml(formatDate(record?.original_expiry_date))}</td>
		                                <td>${escapeHtml(formatDate(record?.revised_expiry_date))}</td>
		                                <td>${escapeHtml(statusPercent)}</td>
		                                <td>${escapeHtml(toDisplay(record?.remarks))}</td>
		                                <td>${escapeHtml(toDisplay(record?.contractor))}</td>
		                            </tr>
	                        </tbody>
